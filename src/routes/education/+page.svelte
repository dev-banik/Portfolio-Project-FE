<script lang="ts">
	...
	function formatPeriod(from: Date | null, to?: Date | null): string {
		...
		const fromDate = from.toLocaleDateString('en-US', options);
		const toDate = to ? to.toLocaleDateString('en-US', options) : 'Present';
				...
				<div class="col flex-1 items-stretch">
					<Card>
						<div class="flex-1 col gap-2 items-stretch">
							...
							<div class="text-[1em]">{education.organization}</div>
							<div class="text-[1em]">{education.description} | {education.name}</div>							
							<div class="col text-[0.9em]">
								...
								{#if education.period}
									<div class="row items-center gap-2">
										...
										{formatPeriod(education.period?.from, education.period?.to)}
									</div>
									<CardDivider />
								{/if}
